...
__device__ inline void cp_async4_stream(void* smem_ptr, const void* glob_ptr) {
  const int BYTES = 16;
  uint32_t smem = static_cast<uint32_t>(__cvta_generic_to_shared(smem_ptr));
  // NOTE: upstream Marlin uses createpolicy.fractional.L2::evict_first.b64 + the
  // .L2::cache_hint variant of cp.async here (an L2-residency hint for weights that are only
  // read once). On sm_120, a ptxas codegen bug silently drops the createpolicy instruction
  // while keeping the dependent cache_hint consumer, which then reads an uninitialized
  // descriptor register and traps with cudaErrorIllegalInstruction at runtime. Since this is a
  // speed-only reference build (see marlin_speed_bench.cu), we drop the cache hint entirely and
  // fall back to a plain cp.async -- functionally equivalent for timing purposes, just without
  // the (unrealized, on this arch) L2-eviction-priority hint.
  asm volatile(
